const TictactoeGame = require('../../database/Database').TictactoeGame;

class TicTacToeUtils {
    constructor(challenger, rival, matches) {
        ...
        const a = setInterval(() => {
            ...
            matches.delete(this.challenger);
            matches.delete(this.rival);
            this.draw = true;
        }, 1000);
        this.interval = a;
        this.draw = false;
            ...
            const interval = setInterval(() => {
                if (count >= 3) {
                    channel.send('***BEGIN!***').then(message => {
                        setTimeout(() => {
                            message.edit('**Playing: ' + this.playing.displayName + '**');
    ...
    makeMove(player, number, channel) {
    ...
    checkPlay(channel) {
        ...
        for (let possibility in winWith) {
            ...
            if (TicTacToeUtils.arrayEqual(os, [true, true, true])) {
                setTimeout(() => {
                    channel.send(`**${this.challenger.displayName} vs. ${this.rival.displayName}**` +
                        `\nWinner: ${this.playing_next.displayName}`);

                    let winner = this.playing_next.id === this.challenger.id ? this.challenger.id : this.rival.id;
                    let looser = this.playing_next.id !== this.challenger.id ? this.challenger.id : this.rival.id;

                    TictactoeGame.saveMatch(winner, looser, this.matchTime, this.table_message.content, this.draw)
                }, 500);
                resetArrays();
                this.current_message.delete();
                this.stopMatch();
                break;
            } else if (TicTacToeUtils.arrayEqual(xs, [true, true, true])) {
                setTimeout(() => {
                    channel.send(`**${this.challenger.displayName} vs. ${this.rival.displayName}**` +
                        `\nWinner: ${this.playing_next.displayName}`);
    ...
    stopMatch() {
        ...
        this.matches.delete(this.challenger);
        this.matches.delete(this.rival);
        this.matchTime = Date.now() - this.matchTime;
    }
